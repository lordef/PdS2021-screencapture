...
void ScreenRecorder::generateVideoStream() //Nome aggiornato
{
    ...
    value = avcodec_open2(outAVCodecContext, pLocalCodec, nullptr);
    ...
    if (outVideoStreamIndex < 0) {
        throw std::runtime_error("Error: cannot find a free stream index for video output");
    }
...
int ScreenRecorder::captureVideoFrames()
{
    ...
    if (!pAVPacket)
        throw std::runtime_error("Error in allocating AVPacket");
    pAVFrame = av_frame_alloc();

    if (!pAVFrame) // Verifichiamo che l'operazione svolta da "av_frame_alloc()" abbia avuto successo
    {
        throw std::runtime_error("Error: Unable to release the avframe resources");
    }

    outFrame = av_frame_alloc(); 
    if (!outFrame)
    {
        throw std::runtime_error("Error: Unable to release the avframe resources for outframe");
    }
    ...
    {
        throw std::runtime_error("Error in filling image array");
    }
    ...
    {
        throw std::runtime_error("Error nell'allocazione del SwsContext");
    }

    value = sws_init_context(swsCtx_, NULL, NULL);
    if (value < 0)
    {
        throw std::runtime_error("Errore nell'inizializzazione del SwsContext");
    }
    ...
    */
    while (!getVideoBool())
    {
...
#elif linux
            string resolutionS = to_string(cropW) + "x" + to_string(cropH);
            av_dict_set(&options, "video_size", resolutionS.c_str(), 0);   
            string url = ":0.0+" + to_string(cropX) + "," + to_string(cropY);
            value = avformat_open_input(&pAVFormatContext, url.c_str(), pAVInputFormat, &options);
#endif
            if (value != 0) {
        ...
        if (av_read_frame(pAVFormatContext, pAVPacket) >= 0 && pAVPacket->stream_index == VideoStreamIndx)
        {
            ...
            {
                throw std::runtime_error("Problem with avcodec_send_packet");
            }
            ...
            {
                throw std::runtime_error("Output not available in this state.  Try to send new input.");
            }
            else if (value < 0)
            {
            ...
            if (value < 0)
            {
                throw std::runtime_error("Problem with sws_scale");
            }
            ...
            {
                cout << "Output not available in this state. Try to send new input" << endl;
                continue;
            }
            ...
            if (value >= 0) // Frame successfully encoded :)
            {
                ...
                {
                    throw std::runtime_error("Error in writing video frame");
                }
            ...
            pAVPacket = av_packet_alloc();
            if (!pAVPacket)
                throw std::runtime_error("Error in allocating pavPacket");

            av_frame_free(&pAVFrame);
            pAVFrame = av_frame_alloc();
            if (!pAVFrame) // Verifichiamo che l'operazione svolta da "av_frame_alloc()" abbia avuto successo
            {
                throw std::runtime_error("Unable to release the avframe resources");
            }
            ...
            if (!outFrame)
            {
                throw std::runtime_error("Unable to release the avframe resources for outframe");
            }
